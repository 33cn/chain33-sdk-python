...
class SM2Util(object):
    ...
    # 生成SM2密钥对
    def genetateKey(self):
        n = self.ecc_table['n']
        nint = int(n, 16)
        d = random.randint(1, nint - 2)
        pub = self._kg(d, self.ecc_table['g'])
        return d, pub

    # 私钥转换为公钥
    def pubKeyFromPrivate(self, priv):
        return self._kg(priv, self.ecc_table['g'])
    ...
    # 签名验证
    def verify(self, data, pub, sign, uid):
        e = self.get_e(data.hex(), pub, uid)
        return self.verify_with_e(sign, pub, bytes.fromhex(e))

    # 消息签名
    def sign(self, data, priv, uid):
        e = self.get_e(data.hex(), self.pubKeyFromPrivate(priv), uid)
        k = random_hex(self.para_len)
        return self.sign_with_e(bytes.fromhex(e), priv, k)

    def sign_with_e(self, data, priv, K):
        E = data.hex()
        e = int(E, 16)
